...
function Back() {
  ...
  return (
    <nav className={`fixed z-50 top-0 left-0 w-screen flex justify-between items-center px-6 py-4  backdrop-filter backdrop-blur-md bg-opacity-30 ${isLight ? 'bg-white shadow-sm text-black ' : 'bg-gray-900 text-white'} `}>
  <div className="font-semibold relative flex items-center justify-center gap-1 capitalize">
    <IoIosArrowBack onClick={() =>setPage("general")} size={30}  className=" cursor-pointer  relative -top-0 mr-0" />
    <img alt='bg' src={img} className='h-[40px] w-[40px] border-2 border-gray-500 rounded-[50%] ' />
    <p className='ml-2'>{room}</p>
  </div>
  <div  className="flex items-center">
    <LuMoreVertical onClick={() =>{
      setMenu(!menu)
    }} size={25} className=" cursor-pointer mr-2" />
  </div>
  <AnimatePresence>{menu && <Options menu={menu} setMenu={setMenu} />}</AnimatePresence>
</nav>
  )
}
const Options = ({menu,setMenu}) =>{
  ...
  exit={{
    ...
    className={`fixed top-16 right-10  flex flex-col px-1 py-2 rounded-[7px] ${isLight? ' bg-white shadow-md text-black' : 'bg-gray-800'} `}> 
          <p  onClick={() =>{
                    setIslight(false)
                    setMenu(false)
